...
- **Test-harness dependency, now a real prerequisite PR**: `supabase/tests/run.sh` originally applied only `supabase/migrations/0001_captivate_core.sql`, not every migration — without a fix, `npm run test:rls` would never see any migration this plan adds (Task 1's provenance columns, Task 2's budget tables), making every RLS check below vacuous. This is now a verified, real fix on branch `claude/fix-rls-harness-migration-coverage` (confirmed end-to-end locally: a regression check fails against the old harness, passes against the fixed one) — merge or rebase onto it before starting this plan's Task 1. If for some reason it hasn't landed yet, apply the same fix directly (loop `for f in supabase/migrations/*.sql; do psql ... -f "$f"; done` instead of the single hardcoded file, plus the `storage.buckets`/`storage.objects` stub addition that branch also needed for `0002_storage.sql` to apply) rather than proceeding with a harness that silently isn't testing anything new.
