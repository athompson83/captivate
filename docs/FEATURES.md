...
| Show-wide backdrop                     | Implemented | One picture behind the whole show, on a plane at a chosen distance so flights move it with depth; dimmed on a scene, whole from the overview; made to the look, or found in stock from the writer's room query where there is no image key |
