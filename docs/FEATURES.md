...
| Show-wide backdrop                     | Implemented | One picture behind the whole show, on a plane at a chosen distance so flights move it with depth; dimmed on a scene, whole from the overview; graded to the deck like every picture; made to the look, or found in stock from the writer's room query where there is no image key                                                                             |
| A room per movement                    | Implemented | A movement can stand in a room of its own, chosen beside its name or found in stock from the writer's words where the argument moves somewhere else; the show stands in it on that movement's scenes and establishing shot, crossfading on the same plane, and in the show's room elsewhere and from the overview; the export and the share resolver carry it |
...
| Share card                         | Implemented | A share link unfurls as the deck's own card, in its theme, with its shape, standing in the deck's room when it has one and it arrives in time |
| Help under `?`                     | Implemented | The editor's shortcut list and the stage's keys, one key away                                                                                 |
| Captions for the room              | Implemented | `T`; the browser's own speech engine, from whichever window has the microphone                                                                |
