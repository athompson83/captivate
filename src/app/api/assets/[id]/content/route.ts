 ...
 *
 * A shared deck can reference the same URLs, and a link-holder is not the
 * owner — signed in or not — so any caller RLS turns away falls back to
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Having a session does not make you the owner. A signed-in visitor
  // following someone else's share link matches no row above, so without this
  // fallback every image on a shared deck 404s for them and resolves fine for
  // a logged-out visitor. The resolver is gated on the deck being shared
  // right now, so this grants an account exactly what a link alone grants —
  // no more.
  if (!storagePath) {
