...
export const ThemeFonts = z.object({
  ...
  /** The print hand that letters a drawing's names. Defaulted, so a stored theme without one still parses. */
  hand: z.string().max(120).default("var(--font-hand)"),
});
...
export function themeCssVars(theme: PresentationTheme): Record<string, string> {
  return {
    ...
    "--stage-font-hand": theme.fonts.hand,
  };
}
