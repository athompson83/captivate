...
>(drawing: T): T {
  ...
  if (bounds) {
    ...
    // would move the picture on its canvas, and the model composed it where
    // it meant to. The stored box is the author's coordinate system — the
    // hand's wobble and the labels are sized against it — and what the room
    // sees is the frame `frameOf` measures from the ink itself, so a stroke
    // at negative coordinates is shown too; it is only not counted here.
