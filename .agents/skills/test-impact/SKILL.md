...
---
# Test Impact
Map changed files/symbols to callers and affected boundaries. Start with the cheapest check that can falsify intended behavior, then broaden for shared code, contracts, schema/persistence, auth/tenancy, security, dependencies, deployment, or release behavior. Classify skipped, unavailable, and not-applicable checks explicitly; never convert missing required evidence into success. Record exact checks and results supporting completion claims.
